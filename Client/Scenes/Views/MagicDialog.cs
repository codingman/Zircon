...
namespace Client.Scenes.Views
{
    ...
    public sealed class MagicTab : DXTab
    {
        ...
        public MagicTab(MagicSchool school)
        {
            ...
            TabButton.Hint = MagicSchoolChinese.GetText(school);
    ...
    public sealed class MagicCell : DXControl
    {
        ...
        public void Refresh()
        {
            ...
            if (MapObject.User.Magics.TryGetValue(Info, out ClientUserMagic magic))
            {
                ...
                {
                    ExperienceLabel.Text = $"要求角色等级: {Info.NeedLevel1}";
                ...
                else
                {
                    if (magic.Level < Globals.MagicMaxLevel)
                    {
                        ...
                        {
                            case 0:
                                ExperienceLabel.Text = $"熟练度: {magic.Experience}/{magic.Info.Experience1}";
                                break;
                            case 1:
                                ExperienceLabel.Text = $"熟练度: {magic.Experience}/{magic.Info.Experience2}";
                                break;
                            case 2:
                                ExperienceLabel.Text = $"熟练度: {magic.Experience}/{magic.Info.Experience3}";
                                break;
                            default:
                                ExperienceLabel.Text = $"熟练度: {magic.Experience}/{(magic.Level - 2) * 500}";
                                break;
                        }
                    }
                    else
                    {
                        ExperienceLabel.Text = $"熟练度: 已达到最高等级";
                    }
